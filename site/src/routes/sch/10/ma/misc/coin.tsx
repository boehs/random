// I would love to have a simulator flip a coin 10 times and record how many heads there are.
...
// Below is an example of a graph like this (but for 100 flips each time)
...
export default function Coin() {
    ...
    const [numTimes, setNumTimes] = createSignal(10)
    const [refreshKey, setRefreshKey] = createSignal(0)
    const [computed,setComputed] = createStore<number[]>([])
    
    createEffect(() => {
        refreshKey(), numTimes(), numFlips()
        setComputed(new Array(numFlips()).fill(0))
        let i = 0;
        const tmout = setInterval(() => {
            i++
            const n = Math.floor(Math.random() * (numTimes() + 1))
            setComputed(n,computed[n] + 1)
            ...
            <For each={computed}>
                {(n,i) => <div style={{"flex": "1", "display": "flex", "flex-direction": "column"}}>
                    <div style={{
                        ...
                        <div style={{"background-color": "var(--pri)", "border-radius": "5px", "border": "2px solid var(--sec)", "height": `${((n / numTimes())) * 100}%`, "width": "100%"}}>{n}</div>
                    </div>
                    {i}
                </div>}
            </For>
